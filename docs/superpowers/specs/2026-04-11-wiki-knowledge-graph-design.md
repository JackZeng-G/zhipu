...
> **状态：基本完成** (2026-04-13)
>
> 7 项实施步骤中 6 项已完成。唯一未完成项"清理旧 wiki_pages"保留兼容性暂不处理。
> 本设计涉及的 confidence 自动晋升、Evolution Log 语义化等细节已在 2026-04-13 增强规格中纳入优化计划。
...
| # | 实施步骤 | 状态 |
|---|---------|------|
| 1 | 数据库：新增 wiki_concepts、concept_relations、questions 表 | **已完成** |
| 2 | Store 层：新增结构体和 CRUD | **已完成** |
| 3 | API 层：重构 INGEST 流程（概念聚合 + 共现关系） | **已完成** |
| 4 | API 层：新增概念页查看/按需生成接口 | **已完成** |
| 5 | 前端：WikiView 重构为图谱 + 概念列表 | **已完成** |
| 6 | 前端：NotesView 实体标签可点击 | **已完成** |
| 7 | 清理旧的重复 wiki_pages 数据 | **保留兼容性，暂不清理** |

**后续优化（已纳入 2026-04-13 增强规格）：**

| 04-11 设计中的原则 | 当前实现 | 04-13 增强计划 |
|-------------------|---------|---------------|
| Confidence 体系 | source_count 自动晋升 high | 改为用户确认晋升 |
| Evolution Log | 只有 source_added | REFLECT 时语义分类（强化/修正/分歧） |
| 输出持久化 | 仅 activity_log | 新增 wiki_outputs 表，QUERY 答案持久化 |
| 矛盾显式标注 | LLM 生成时检测 | REFLECT Stage 0 反向检验增强 |
